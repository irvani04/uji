...
@section('space-work')
                    ...
                    @foreach ($qna as $key => $data)
                        <div>
                            ...
                            @foreach ($data['question'][0]['answers'] as $answer)
                                <p><br>{{ $acount++ }}).</b>{{ $answer['answer'] }}
                                    <input type="radio" name="radio_{{ $key + 1 }}" data-id="{{ $key + 1 }}"
    ...
    <script>
        ...
        function isValid() {
            ...
            var qlength = parseInt("{{ count($qna) }}");
            $('.error_msg').remove();
            for (let i = 1; i <= qlength; i++) {
                if ($('#ans_' + i).val() == "") {
                    result = false;
                    $('#ans_' + i).parent().append(
                        '<span style="color:red;" class="error_msg"> Please Choice The Answer </span>');
                    setTimeout(() => {
                        $('.error_msg').remove();
                    }, 5000);
                }
